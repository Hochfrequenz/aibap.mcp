...
- **Reproducer verification** (bump-time): adtler bump PRs run the reproducer snippet from each linked `blocked-by-adtler` issue against the live target system(s). This is narrower than adtler's integration suite — one MCP tool call per fix claim — and is the only live test we run at the aibap.mcp boundary. Format and flow: see "Cross-Repo Issue Tracking (adtler)".
- **Fix before creating:** When an SAP object (transport, program, etc.) has a problem, fix the existing one first. Don't keep creating new objects to work around issues.
...
- The tool forwards pre-marshaled JSON bytes from adtler: wrap in `json.RawMessage` so it round-trips through `NewToolResultJSON` without base64-encoding. No typed Go struct exists to generate a schema from. **This only applies when the adtler call genuinely returns JSON.** `tools/debugger.go` used to be the example (`debug_step`/`debug_get_variable`/`debug_get_stack`/`debug_set_watchpoint`), on the wrong assumption that the underlying ADT debugger endpoints return JSON — they return `application/xml`/`text/plain`, so `json.RawMessage` there either failed `NewToolResultJSON`'s marshal step outright (most XML/text bodies aren't valid JSON at all) or — for a `text/plain` value that happened to look like a bare JSON scalar, e.g. `"42"` or `"true"` — marshaled successfully into a **scalar** `structuredContent`, the #351 bug class rather than a marshal error (issue #501; both failure modes are the same underlying bug as #433's `debug_get_sessions`). Those four now use typed structs instead. If you're about to reach for `json.RawMessage`, first confirm the adtler call's `Accept` header is actually `application/json` (or equivalent) — if it's XML or text, wrap the bytes in a named struct field instead, the way `DebugStepResult`/`DebugStackResult`/`DebugWatchpointResult`/`DebugVariableResult` do.
...
- **[`adtler`](https://github.com/Hochfrequenz/adtler)** — the SAP ADT client library this server wraps. Most bug fixes for the SAP-touching code belong here, not in `aibap.mcp`. Already referenced above under Testing, Project Structure, and SAP ADT.
- **[`sap-mcp-config`](https://github.com/Hochfrequenz/sap-mcp-config)** — shared config schema for `systems.json`, consumed by both `aibap.mcp` (Go) and [`sapgui.mcp`](https://github.com/Hochfrequenz/sapgui.mcp) (Python). If you touch config loading, coordinate changes across both consumers.
- **[`sapgui.mcp`](https://github.com/Hochfrequenz/sapgui.mcp)** — complementary Python MCP server that drives SAP GUI and SAP Web GUI. Formerly `sapwebgui.mcp`; the old URL still redirects, so stale references fail silently rather than 404. Users often run it alongside `aibap.mcp` for operations ADT cannot handle: abapGit pull (via [`Z_ABAPGIT_PULL_MCP_SHORTCUT`](https://github.com/Hochfrequenz/Z_ABAPGIT_PULL_MCP_SHORTCUT)), customizing table maintenance screens, and ECC-only workflows such as transport release via `SE09` (see the SAP ADT note above). The two MCPs share `~/.config/sap-mcp/systems.json`. When a user asks this server to do something that is fundamentally a GUI-only workflow, the honest answer is usually "use `sapgui.mcp` for that step" — do not try to fake it via BlackMagic unless there is already a proven fallback path.
- **[`AIBAP_TEMPLATE_REPOSITORY`](https://github.com/Hochfrequenz/AIBAP_TEMPLATE_REPOSITORY)** — the GitHub template users start from when they set up an AI-driven ABAP vibe coding project. Its README presents the ADT workflow (powered by `aibap.mcp`) and the abapGit workflow (powered by `sapgui.mcp`) as two peer options with a comparison table. If you rename tools, change the `--tools` flag semantics, change supported `create_object` types, or break tool contracts, grep the template repo for stale references and fix them in the same change.
