...
> classrun load-generation defect — a class freshly created/activated via ADT,
> or changed + re-activated, may return "does not implement main" or a stale
> load until its runtime load is generated by instantiating it once outside
> classrun (Eclipse "Run as ABAP Application", or a report doing `CREATE
> OBJECT`). Confirmed live on S4U (SAP_BASIS 758). Remove that paragraph from
