...
> **Post-ship note (adtler#106, resolved):** the temporary "Known limitation"
> workaround paragraph has been removed from the tool description. adtler#106 is
> fixed — `RunClass` now runs each classrun on an isolated fresh session
> (adtler#107), which generates the load from the current active source and
