...
> **Post-ship note (adtler#106, temporary):** the shipped `WithDescription`
> additionally carries a "Known limitation" workaround paragraph for the
> classrun load-generation defect — on **S/4**, a class freshly created/activated
> via ADT, or changed + re-activated, may return "does not implement main" or a
> stale load until its runtime load is generated by instantiating it once outside
> classrun (Eclipse "Run as ABAP Application", or a report doing `CREATE
> OBJECT`). **S/4-specific:** verified 2026-07-27 that on ECC/R3 (HFQ) activation
> regenerates the load and `run_class` works correctly with no workaround; both
> defects reproduce only on S/4 (S4U, SAP_BASIS 758). Remove that paragraph from
> the tool description (and the spec note) once adtler#106 is fixed.
