...
// lock_object call).
//
// autoLocked reports whether this call newly acquired the lock — either the
// forced ECC relock, or a cache-miss auto-lock on the fallback path — so
// callers know whether to roll it back via releaseAutoLock on a subsequent
// write failure.
func resolveWriteLockHandle(ctx context.Context, client writeLockClient, lockMap *adt.LockMap, tracker *sessionLockTracker, key, uri, explicitHandle string, allowAutoLock bool) (handle string, autoLocked bool, err error) {
	if explicitHandle != "" {
		return explicitHandle, false, nil
	}

	if flavor, ferr := client.SystemFlavor(ctx); ferr == nil && flavor == adt.SystemFlavorECC {
		h, err := client.LockObject(ctx, uri)
		if err != nil {
			return "", false, fmt.Errorf("auto-lock %s: %w", uri, err)
		}
	...
	if !allowAutoLock {
		...
		if !tracked || state.LockHandle == "" {
			return "", false, fmt.Errorf("no lock tracked for %s in this session — call lock_object first", uri)
		}
		return state.LockHandle, false, nil
	}
